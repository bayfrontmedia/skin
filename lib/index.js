...
const twPlugin = plugin(function ({addUtilities, addComponents, addBase, theme, config}) {
    ...
    /**
     * Return 0.25rem when the "borderRadius" Skin config value is > 0.
     * Used for elements which will only have a small border radius no matter
     * the borderRadius config value.
     *
     * @returns {string}
     */
    function getMinimumBorderRadius() {

        let radius = skinConfig('borderRadius', '.25rem');

        let parseNum = str => +str.replace(/[^.\d]/g, '');

        if (parseNum(radius) > 0) {
            return '0.25rem';
        }
    ...
    addComponents({
        ...
        '.tc-form-checkbox,.tc-form-radio': {
            ...
            borderRadius: getMinimumBorderRadius()
        },
        '.tc-form-radio': {
            '@apply rounded-full': {}
        },
