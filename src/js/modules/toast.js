...
export function show(config = {}, lang = {}) {
    ...
    config.text = '<div class="text-sm">' + config.text + '</div><button onclick="App.Toast.close(this)" type="button" class="ms-auto size-6 ac-btn-close" aria-label="' + lang.close + '"><span class="sr-only">' + lang.close + '</span><span class="iconify heroicons--x-mark-solid text-base"></span></button>';
    config.className = config.className + " tc-toast hs-toastify-on:opacity-100 [&>.toast-close]:hidden";
